...
{
    /// <summary>
    /// Selects mates for a reference individual by applying a sequence of
    /// <see cref="MatchingSettings"/> pick directives, with optional per-scope caching
    /// of the picks. This is the engine behind <see cref="MatchMetaHeuristic"/>.
    /// </summary>
    public class MatchPicker
    {
        ...
        /// <summary>
        /// Exposes the protected static roulette-wheel routines of the upstream
        /// <see cref="RouletteWheelSelection"/> so the wheel can be built once per scope
        /// and reused across picks.
        /// </summary>
        ...
        /// <summary>
        /// The metaheuristic producing offspring on the fly for <see cref="MatchingKind.Child"/> picks.
        /// </summary>
        public IMetaHeuristic ChildMetaHeuristic { get; set; }

        /// <summary>
        /// Pick-directive pipeline for <see cref="MatchingKind.Custom"/>: each inner list is
        /// applied to the output of the previous step.
        /// </summary>
            ...
            IList<IChromosome> parents)
        {
            ...
            switch (currentMatchingSettings.MatchingKind)
            {
                ...
                case MatchingKind.Best:
                    ...
                    {
                        // The PR read CurrentGeneration.BestChromosome, filled by Generation.End().
                        // The autonomous engine never calls End() (no implicit sort), so the
                        // population-level best is the authoritative source here.
                        if (ctx.Population.BestChromosome != null)
                        {
                            selectedParents.Add(ctx.Population.BestChromosome);
                        }
                        else
                        {
                ...
                case MatchingKind.Child:
                    ...
                    for (int i = 0; i < currentMatchingSettings.AdditionalPicks + 1; i++)
                    {
                        ...
                        {
                            throw new InvalidOperationException("child match heuristic didn't produce any offspring");
                        }
