...
{
    /// <summary>
    ///   The Forensic-Based Investigation algorithm, a compound metaheuristic inspired by
    ///   the suspect investigation-location-pursuit process used by police officers.
    ...
    ///   (Jui-Sheng Chou, Ngoc-Mai Nguyen, 2020) and implemented directly from
    ///   <see href="https://github.com/thieu1995/mealpy/blob/master/mealpy/human_based/FBIO.py">the mealpy version</see>.
    ///   This is the default version of the algorithm with reduced lambda expressions.
    ...
    public class ForensicBasedInvestigation : GeometricMetaHeuristicBase
    {
        ...
        protected virtual object A1StepOperator(int geneIndex, IEnumerable<object> geneValues, IGeometricConverter geometricConverter, int nchange)
        {
            ...
            // ADAPTATION: upstream IRandomization (tag 3.1.4) exposes no GetNormal; the mealpy FBIO
            // reference uses N(0,1) noise here, reproduced via a Box-Muller transform on GetDouble.
            double u1 = Math.Max(rnd.GetDouble(), 1e-10);
            double u2 = rnd.GetDouble();
            double normal = Math.Sqrt(-2.0 * Math.Log(u1)) * Math.Cos(2.0 * Math.PI * u2);
            var geometricValue = metricValues[0] + normal * (metricValues[0] - (metricValues[1] + metricValues[2]) / 2);
        ...
        protected override IContainerMetaHeuristic BuildMainHeuristic()
        {
            ...
            // ADAPTATION: MetaPopulation never calls Generation.End(), so the generation-level
            // BestChromosome field is never set (the order-preserving engine derives best from
            // Chromosomes by fitness instead, cf. MetaPopulation.EndCurrentGeneration comment).
            // PR #87 read ctx.Population.CurrentGeneration.BestChromosome.Fitness here; MGS derives
            // the current-generation best via Chromosomes.MaxBy(Fitness).
            var a2StepHeuristic = new IfElseMetaHeuristic().WithName("A2 Step")
                .WithScope(EvolutionStage.Crossover)
                .WithParam(nameof(FBIParam.pBest), "best fitness of current generation", ParamScope.MetaHeuristic | ParamScope.Generation, (h, ctx) => ctx.Population.CurrentGeneration.Chromosomes.MaxBy(c => c.Fitness.Value).Fitness.Value)
                ...
                .WithCaseGenerator(ParamScope.None, (IMetaHeuristic h, IEvolutionContext ctx, double prob) => GetA2Switch(prob))
            ...
            var b2StepHeuristic = new MatchMetaHeuristic().WithName("B2 Step")
                .WithParam(nameof(FBIParam.randomBetter), "random better than current", ParamScope.MetaHeuristic | ParamScope.Generation | ParamScope.Individual, (h, ctx) => ctx.SelectedParents[0].Fitness.Value > ctx.SelectedParents[1].Fitness.Value)
