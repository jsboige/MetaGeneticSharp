...
{
    /// <summary>
    ///   Builds an <see cref="IslandMetaHeuristic"/> from a list of compound metaheuristics, so that
    ///   each island runs a different compound (a heterogeneous archipelago). Three construction shapes:
    ///   (1) a fixed island size repeated evenly over a list of compounds,
    ///   (2) a total population split across compounds by shares,
    ///   (3) an explicit per-island (size, compound) list.
    ...
    public class IslandCompoundMetaheuristic : ICompoundMetaheuristic
    {
        ...
        public IContainerMetaHeuristic Build()
        {
            ...
            var islandMetaHeuristic = new IslandMetaHeuristic(phaseheuristics)
            {
                GlobalMigrationRate = GlobalMigrationRate,
                MigrationMode = MigrationMode,
                MigrationsGenerationPeriod = MigrationsGenerationPeriod
            };
