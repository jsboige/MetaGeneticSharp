...
**Prerequisite — Geometric infrastructure (ships first):** the Geometric verbs deferred from Phase 3 (`WithLinear/GeneralGeometricOperator`/`WithGeometryEmbedding`) plus `GeometricConverter<T>`/`TypedGeometricConverter`/`IGeometryEmbedding` (the PR's `Domain.Crossovers.Geometric`) land here first — the compound metaheuristics parametrize over a gene↔metric embedding, so the converter is load-bearing, not optional. **Registry cap:** `MetaHeuristicsService` (12036, moved here from Phase 3) and its `KnownCompoundMetaheuristics` enum are the named-factory layer on top of the compound implementations — port them once WOA/EO/FBI exist, so the factory instantiates real compounds rather than stubs.
